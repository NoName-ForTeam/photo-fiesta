...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3510
        NAMESPACE = "photo-fiesta-online"
        REGISTRY_HOSTNAME = "nonameforteam"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "photo-fiesta"
        DEPLOYMENT_NAME = "photo-fiesta-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'photo-fiesta-online') {
